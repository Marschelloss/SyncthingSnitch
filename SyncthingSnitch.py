...
def fetch_events(args, last_id):
    ...
    if args.ssl:
        proto = 'https://'
    url = proto + args.host + "/rest/events/disk" + "?since=" + str(last_id) + "&timeout=" + str(args.timeout)
...
def send_event(event, bot, args):
    msg_text = "🕵️‍♂️  Meine Spürnase hat frische Ware entdeckt\! 👀\n\nLabel: `%s` \- File: `%s`" % (event['data']['label'], event['data']['path'])
    # Send message, retry three times on error
    for _ in range(3):
        try:
            bot.sendMessage(chat_id=args.telegram_chat_id,
                            text=msg_text,
            ...
            logger.info("Sending event '%d' with path '%s' succeed." % (event['id'], event['data']['path']))
            break
    else:
...
def main(args):
    ...
    logger.info("Starting loop. Press CTRL+C to stop program.")
    try:
        while True:
...
if __name__ == '__main__':
    ...
    # FilterMovies
    parser.add_argument("--filter_movies",
                        action="store_true",
                        default=False,
                        help="Only show events matching movie files")

    # ID File Location
    parser.add_argument("--id_file_location",
                        action="store",
